...
2. Render reads `render.yaml` and creates a free Web Service named `sightlinescan-api` rooted at `server/`. The build runs `npm install && rm -rf .cache/puppeteer && npx puppeteer browsers install chrome`.
...
(No Blueprint? Create the Web Service manually: root directory `server`, build command `npm install && rm -rf .cache/puppeteer && npx puppeteer browsers install chrome`, start command `npm start`.)

**Where Chrome lives, and why it matters.** By default Puppeteer downloads Chrome to `~/.cache/puppeteer`. On a host like Render the build step and the running server don't necessarily see the same home directory, so Chrome gets installed during the build and then the server can't find it (`Could not find Chrome (ver. …)`). `server/.puppeteerrc.cjs` fixes this by pinning the cache to `server/.cache/puppeteer` — inside the project directory, right next to `node_modules`, which definitely survives from build to runtime. Both the build-time installer and the running server read that same config file, so they can't disagree.

**Pin the Node version.** `server/.node-version` pins Node 22. This matters: if you only specify a loose range like `>=20` in `engines`, Render installs the newest Node that satisfies it (Node 26 at time of writing), and Puppeteer 23 doesn't run on it. The failure is confusing rather than obvious — `npx puppeteer browsers install chrome` crashes partway, leaving an empty browser folder behind, so the errors you get are "could not find Chrome" and "browser folder exists but the executable is missing" rather than anything mentioning Node.

Do **not** set a `PUPPETEER_CACHE_DIR` environment variable on the service. Env vars override the config file, which reintroduces exactly this bug. The `rm -rf` in the build command is there because Puppeteer's installer treats "folder already exists" as "already installed" without verifying it's complete, so a half-downloaded folder from a previous build would otherwise be reused forever.

Remember: editing `render.yaml` in the repo does **not** update an already-created service's saved settings. Change the Build Command directly in the Render dashboard under Settings → Build & Deploy to match.
